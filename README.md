# @shapeshift-labs/frontier-queue

Serializable durable queue state, leases, retries, dedupe, dead-letter records, and replay evidence for Frontier apps.

`frontier-queue` turns queue behavior into Frontier data: jobs carry payloads or patch tuples, workers lease jobs with expiring tokens, failures produce retry or dead-letter records, and every mutation returns a Frontier patch plus replay evidence. It is not a broker, database, scheduler, or worker runtime. Those systems attach through structural adapters.

## Terminal Outcomes

Queues keep durable terminal outcomes for dedupe keys that complete, cancel, or dead-letter. A later stale manifest with the same dedupe key is recorded as `terminal-deduped` instead of resurrecting old work. Explicit operator retries still go through `retryQueueDeadLetter`, which requeues the original job intentionally.

Use `projectQueueTerminalOutcomes` to import terminal outcomes from older queue snapshots, dead letters, or another machine before accepting new enqueue manifests.

```ts
import { enqueueQueueJob, projectQueueTerminalOutcomes } from '@shapeshift-labs/frontier-queue';

const projected = projectQueueTerminalOutcomes(queueState, { states: [remoteQueueState] });
const mutation = enqueueQueueJob(projected.state, {
  dedupeKey: 'task:build-assets',
  payload: { source: 'stale-manifest' }
});
```
...
```sh
npm install @shapeshift-labs/frontier-queue
```
...
const enqueued = enqueueQueueJob(queue, {
  queue: 'media',
  payload: { assetId: 'asset:hero.jpg', size: 256 },
  patch: [[0, ['assets', 'hero', 'status'], 'queued']],
  dedupeKey: 'thumb:hero.jpg:256',
  dedupeMode: 'drop',
  groupKey: 'asset:hero.jpg',
  priority: 10
}, { now: Date.now() });
queue = enqueued.state;

const leased = leaseQueueJobs(queue, {
  queue: 'media',
  workerId: 'worker-1',
  count: 1,
  now: Date.now()
...
const failed = job
  ? failQueueJob(queue, {
      jobId: job.id,
      error: { type: 'TransientAssetError', message: 'retry later', retryable: true }
    })
  : undefined;
```
...
- `createQueueState` creates a serializable queue snapshot with defaults for queue name, leases, stalls, retry policy, and dead-letter queue.
- `enqueueQueueJob` stores payload or patch-carrying jobs and supports `drop`, `replace`, `preserve-run-at`, `unsafe-dedupe`, and `merge-payload` dedupe modes.
- `leaseQueueJobs` selects ready jobs by priority, availability, creation time, and stable id while respecting per-queue `groupKey` concurrency.
- `completeQueueJob`, `failQueueJob`, `expireQueueLeases`, `retryQueueDeadLetter`, and `cancelQueueJob` produce compact Frontier patch tuples and queue event records.
- `inspectQueueState`, `createQueueEvidence`, and `verifyQueuePatch` provide replayable evidence and health counts.
- `encodeQueueJsonl`, `decodeQueueJsonl`, `stableQueueStringify`, `hashQueueValue`, and `calculateRetryDelayMs` support persistence logs, deterministic backoff, and proof hashes.

## Boundary

The root import depends only on `@shapeshift-labs/frontier`. Broker, SQL, IndexedDB, filesystem, worker, workflow, scheduler, event-log, effects, policy, and trace integrations should stay structural so apps can own their runtime choices.

`@shapeshift-labs/frontier-worker` describes work that may run in many runtimes. `@shapeshift-labs/frontier-queue` owns the durable queue state machine those runtimes can adapt to: enqueue, lease, retry, dead-letter, replay, and inspect.
...
The benchmark writes `benchmarks/results/frontier-queue-package-bench-latest.json` when run from the monorepo. These are Frontier-only package measurements for enqueue, dedupe, priority leasing, group-key leasing, retry, dead-letter, lease expiry, inspection, JSONL, and evidence hashing.

## Source Repository

- GitHub: https://github.com/siliconjungle/-shapeshift-labs-frontier-queue
